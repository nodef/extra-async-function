...
async function backoffRec(x: Function, as: any[], e: any, n: number, N: number, t: number, T: number, tf: number): Promise<any> {
  if(N>=0 && n>=N) throw e;
  if(T>=0 && t>=T) throw e;
  try { return x(...as, e); }
  catch(e) { await WAIT(t); await backoffRec(x, as, e, n+1, N, t*tf, T, tf); }
}
